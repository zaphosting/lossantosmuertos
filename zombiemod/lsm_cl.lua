	...
	Citizen.CreateThread(function()
		...
		while true do
			...
			if guard_invincible_1~=invincible_1 then
				...
				--if guard_debug then
				if invincible_1 and anticheat_on == 1 then
			...
			if invincible_1 and timestamp-guard_invincibility_timestamp>2000 and anticheat_on == 1 then
				--print(timestamp..":DETECTED invincibility type 1 ENABLED")
				_tse(event.debug,code_inv1)
			end
			if guard_invincible_2~=invincible_2 and anticheat_on == 1 then
			...
			-- -- end
			if guard_model~=model and anticheat_on == 1 then
			...
			if guard_max_health~=max_health and anticheat_on == 1 then
			...
			if guard_health~=health and anticheat_on == 1 then
			...
			if guard_z>-192.0 and not attached then
				...
				if detect and anticheat_on == 1 then
			...
			if guard_armor~=armor and anticheat_on == 1 then
			...
			if guard_tagscanner==0x7FF00 and anticheat_on == 1 then
			...
			if guard_explosions~=explosions and anticheat_on == 1 then
			...
			if guard_player_blips~=blips and anticheat_on == 1 then
			...
			if guard_wanted~=wanted then
				...
				elseif wanted<guard_wanted then --and timestamp-guard_wanted_timestamp<1000 then
					if timestamp-guard_wanted_got_timestamp<39000 and guard_wanted_flashed_last_time==false and anticheat_on == 1 then
			...
			if guard_max_wanted~=max_wanted and anticheat_on == 1 then
			...
			if guard_wanted_set_timestamp~=0 and timestamp-guard_wanted_set_timestamp>10000 and anticheat_on == 1 then
			...
			if shooting and weapon~=911657153 then --not for tazer
				...
				if bool and clip==guard_clip and GetMaxAmmoInClip(ped,weapon,1)~=1 then
					...
					if det_infcl>50 and anticheat_on == 1 then
				...
				if bool and ammo==guard_ammo then
					...
					if det_infam>50 and anticheat_on == 1 then
			...
			then
				...
				if not explosive_ammo_detected and timestamp-last_shot<500 then
					...
					if explosions>0 then
						...
						if last_attack_was_melee and anticheat_on == 1 then
			...
			if camera~=-1 and camera~=guard_legal_camera then
				...
				if math.abs(normcampos.x-campos.x)+math.abs(normcampos.y-campos.y)+math.abs(normcampos.z-campos.z)>.001 then
					if guard_camera~=camera and anticheat_on == 1 then
						guard_camera=camera
			...
			if banned_peds~=nil then
				...
				if guard_peds~=peds and anticheat_on == 1 then
					guard_peds=peds
			...
			if banned_objects~=nil then
				...
				if guard_objects~=objects and anticheat_on == 1 then
			...
			if banned_vehicles~=nil then
				...
				if guard_vehicles~=vehicles and anticheat_on == 1 then
...
local item_descriptions={
	...
	radio="When equipped, the radio shows both the position and alignment of other radio-equipped players. Friendly faction members broadcast at longer ranges than hostile players.",
